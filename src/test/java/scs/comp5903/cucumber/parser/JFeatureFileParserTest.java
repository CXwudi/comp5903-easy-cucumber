...
class JFeatureFileParserTest {
  ...
  void canParseScenario() throws URISyntaxException {
    var jFeatureDetail = new JFeatureFileParser(detailBuilder).parse(ResourceUtil.getResourcePath("sample/jfeature/rummikub/declaring_winner.jfeature"));
  ...
  void canParseScenarioOutline() throws URISyntaxException {
    var jFeatureDetail = new JFeatureFileParser(detailBuilder).parse(ResourceUtil.getResourcePath("sample/jfeature/rummikub/initial_points.jfeature"));
  ...
  void shouldThrowOnEmptyOrNoTitleFeatureFile() {
    var exp = assertThrows(EasyCucumberException.class, () -> new JFeatureFileParser(detailBuilder).parse(ResourceUtil.getResourcePath("sample/jfeature/empty-file.jfeature")));
  ...
  void shouldNotThrowOnFeatureFileWithZeroScenarios() throws URISyntaxException {
    var jFeatureDetail = new JFeatureFileParser(detailBuilder).parse(ResourceUtil.getResourcePath("sample/jfeature/zero-scenarios.jfeature"));
  ...
  void canIgnoreCommentsAndDescription() throws URISyntaxException {
    var jFeatureDetail = new JFeatureFileParser(detailBuilder).parse(ResourceUtil.getResourcePath("sample/jfeature/multi-line-description.jfeature"));
